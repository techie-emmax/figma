import React from "react";
import Navbar from "../Components/Navbar";
import Footer from "../Components/Footer";

const Blog = () => {
  return (
    <div className="mt-10 mt-[#FFFFFF] bg-[#F1F1F1] w-full">
    <div className="flex flex-col items-center justify-center">
      <p className="font-bold font-display text-[49px] text-center">
        Our Blog
        <span className="block text-[20px] text-[#6F7681] font-regular font-display">
          We're constantly pushing the boundaries of what's <br />
          possible and seeking new ways to improve our <br />
          services.
        </span>
      </p>
      <span className="flex items-center justify-center gap-5 m-10">
        <img src="src/assets/Frame 1000008277.png" alt="" />
        <p className="font-regular font-display text-[20px] bg-[#FFFFFF] px-10 py-10">
          May 30, 2023
          <span className="block font-semibold font-display text-[49px] leading-tight mt-10">
            5 reasons why Techty <br />
            is the go-to choice for <br />
            small business.
          </span>
          Let us manage your IT for you so that you can get back <br />
          to doing what you do best.
        </p>
      </span>
    </div>
    <div className="">
    <p className="font-bold font-display text-[49px] text-center">
        Read all Article
        <span className="block text-[20px] text-[#6F7681] font-regular font-display">
          We're constantly pushing the boundaries of what's <br />
          possible and seeking new ways to improve our <br />
          services.
        </span>
      </p>
      <div className="grid grid-cols-3 grid-rows-2 items-center justify-center gap-2 m-10">
        <span className="flex flex-col items-center justify-center">
          <img src="src/assets/Frame 1000008215.png" alt="" />
          <p className="font-regular font-display text-[18px] py-2 px-5 text-[#6F7681] bg-[#FFFFFF]">May 30, 2023
          <span className="block font-semibold font-display text-[20px] leading-tight mt-10">
            5 reasons why Techty
            is the go- <br />to choice for 
            small business.
          </span>
          Let us manage your IT for you so that you <br />can get back
          to doing what you do best.
        </p>
        </span>
        <span className="flex flex-col items-center justify-center">
          <img src="src/assets/Frame 1000008215 (1).png" alt="" />
          <p className="font-regular font-display text-[18px] py-2 px-5 text-[#6F7681] bg-[#FFFFFF]">May 30, 2023
          <span className="block font-semibold font-display text-[20px] leading-tight mt-10">
            5 reasons why Techty
            is the go- <br />to choice for 
            small business.
          </span>
          Let us manage your IT for you so that you <br />can get back
          to doing what you do best.
        </p>
        </span>
           <span className="flex flex-col items-center justify-center">
          <img src="src/assets/Frame 1000008215 (2).png" alt="" />
          <p className="font-regular font-display text-[18px] py-2 px-5 text-[#6F7681] bg-[#FFFFFF]">May 30, 2023
          <span className="block font-semibold font-display text-[20px] leading-tight mt-10">
            5 reasons why Techty
            is the go- <br />to choice for 
            small business.
          </span>
          Let us manage your IT for you so that you <br />can get back
          to doing what you do best.
        </p>
        </span>
        <span className="flex flex-col items-center justify-center">
          <img src="src/assets/Frame 1000008215.png" alt="" />
          <p className="font-regular font-display text-[18px] py-2 px-5 text-[#6F7681] bg-[#FFFFFF]">May 30, 2023
          <span className="block font-semibold font-display text-[20px] leading-tight mt-10">
            5 reasons why Techty
            is the go- <br />to choice for 
            small business.
          </span>
          Let us manage your IT for you so that you <br />can get back
          to doing what you do best.
        </p>
        </span>
        <span className="flex flex-col items-center justify-center">
          <img src="src/assets/Frame 1000008215 (1).png" alt="" />
          <p className="font-regular font-display text-[18px] py-2 px-5 text-[#6F7681] bg-[#FFFFFF]">May 30, 2023
          <span className="block font-semibold font-display text-[20px] leading-tight mt-10">
            5 reasons why Techty
            is the go- <br />to choice for 
            small business.
          </span>
          Let us manage your IT for you so that you <br />can get back
          to doing what you do best.
        </p>
        </span>
           <span className="flex flex-col items-center justify-center">
          <img src="src/assets/Frame 1000008215 (2).png" alt="" />
          <p className="font-regular font-display text-[18px] py-2 px-5 text-[#6F7681] bg-[#FFFFFF]">May 30, 2023
          <span className="block font-semibold font-display text-[20px] leading-tight mt-10">
            5 reasons why Techty
            is the go- <br />to choice for 
            small business.
          </span>
          Let us manage your IT for you so that you <br />can get back
          to doing what you do best.
        </p>
        </span>
      </div>
    </div>
    </div>
  );
};

export default Blog;
